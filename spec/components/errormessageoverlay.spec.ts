  ...
  describe("configure", () => {
    ...
    it("adds an event listener for source loaded event", () => {
      ...
      expect(onSpy).toHaveBeenCalledWith(
        playerMock.exports.PlayerEvent.SourceLoaded,
        expect.any(Function)
      );
    });
    ...
    it("adds an event listener for error event when not mobile v3", () => {
      ...
      expect(onSpy).toHaveBeenCalledWith(
        playerMock.exports.PlayerEvent.Error,
        expect.any(Function)
      );
    });
    ...
    describe("mobile v3 handling", () => {
      ...
      beforeEach(() => {
        ...
        (playerMock.exports.PlayerEvent as any).SourceError =
          MobileV3PlayerEvent.SourceError;
        (playerMock.exports.PlayerEvent as any).PlayerError =
          MobileV3PlayerEvent.PlayerError;
        (playerMock.exports.PlayerEvent as any).PlaylistTransition =
          MobileV3PlayerEvent.PlaylistTransition;
      });
      ...
      it("uses message from the error event when mobile v3", () => {
        const setTextSpy = jest.spyOn(
          errorMessageOverlay["errorLabel"],
          "setText"
        );
