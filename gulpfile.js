...
var outputnames = {
  globalNamespace:
    (argv.outputnames && argv.outputnames.globalNamespace) ||
    "bitmovin.playerui",
  filename:
    (argv.outputnames && argv.outputnames.filename) || "bitmovinplayer-ui",
...
gulp.task("fonts", function () {
  return gulp.src(paths.source.fonts).pipe(gulp.dest(paths.target.fonts));
});
...
gulp.task(
  "build",
  gulp.series("clean", gulp.parallel("html", "browserify", "fonts", "sass"))
);
...
gulp.task(
  ...
  gulp.series("build", function () {
    ...
    // Watch FONTS files
    gulp.watch(paths.source.fonts).on("change", gulp.series("fonts"));
